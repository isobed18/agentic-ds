...
export function GuidedPipeline({ runId, profile, workspace, componentOutputs, runStatus, busy, onRun, onPause, onRetry, onAdvanced, onOpenExecutions }: GuidedPipelineProps) {
  ...
  return <PanCanvas>
    <div className="flex min-w-[1530px] items-center gap-5 px-8 py-20">
      <GuidedNode title={t("Data understood")} subtitle={t("Intake, routing, and synthesis complete")} status="succeeded" artifactCount={componentOutputs.flatMap((output) => output.artifact_ids).length} onClick={() => setSelected("summary")} />
      <Arrow active={false} complete />
      {groups.map((group, index) => {
        const status = groupStatus(group.nodes, activeStatus, index);
        const artifactCount = group.stages.reduce((sum, stage) => sum + (artifactIdsByStage.get(stage)?.length ?? 0), 0);
        return <div key={group.id} className="contents"><GuidedNode title={t(group.title)} subtitle={group.nodes.length ? group.nodes.map((node) => t(node.label ?? node.id.replaceAll("_", " "))).join(" · ") : group.stages.map((stage) => t(stage.replaceAll("_", " "))).join(" · ")} status={status} artifactCount={artifactCount} onClick={() => void inspectGroup(group.id)} />{index < groups.length - 1 && <Arrow active={status === "running" || status === "retry"} complete={isSucceeded(status)} />}</div>;
      })}
    </div>
...
function ArtifactModal({ preview, onClose }: { preview: ArtifactPreview; onClose: () => void }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-ink/30 p-4" role="dialog" aria-modal="true"><div className="max-h-[82vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-surface p-5 shadow-2xl"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-wide text-brand-600">{String(preview.artifact_type).replaceAll("_", " ")}</p><h3 className="mt-1 text-lg font-semibold text-ink">{preview.title ? local(preview.title) : t("Artifact details")}</h3></div><button type="button" className="btn-ghost !px-2 !py-1" onClick={onClose}>×</button></div>{preview.summary && <p className="mt-4 text-sm leading-relaxed text-ink-mute">{local(preview.summary)}</p>}{preview.findings?.length ? <ul className="mt-4 space-y-2">{preview.findings.map((finding) => <li key={finding.en} className="rounded-lg bg-surface-sunken px-3 py-2 text-xs text-ink-soft">{local(finding)}</li>)}</ul> : <Empty title={t("Artifact recorded")} hint={t("Open its stage inspection for measurements and provenance.")} />}</div></div>; }
