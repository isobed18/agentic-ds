...
import { useDiagnosticIds, useShowDiagnostics } from "../lib/diagnostics";
...
// #380: these are px handed straight to the elk layout, so they do not follow
// the root the way the node's own text does. Grown by the same 10% as
// everything else -- left alone, the captions inside them would clip.
const NODE_WIDTH = 246;
const NODE_HEIGHT = 90;
const BRANCH_WIDTH = 275;
const BRANCH_HEIGHT = 101;
...
function ComponentNode({ data, selected }: NodeProps<ComponentFlowNode>) {
  ...
  return (
    <article className={cx("h-[5.125rem] w-56 rounded-xl border bg-surface shadow-card", selected ? "border-brand-500 ring-2 ring-brand-100" : "border-line", !component.enabled && "opacity-55")}>
      ...
      <div className="flex h-full items-center gap-3 overflow-hidden rounded-xl px-3.5 py-3">
        ...
        <span className="min-w-0 flex-1"><span className="block truncate text-2xs font-semibold text-ink">{local(component.title)}</span><span className="mt-1 block text-3xs font-medium text-ink-mute">{t(status)}</span></span>
...
function BranchNode({ data, selected }: NodeProps<BranchFlowNode>) {
  return (
    <article className={cx("h-[5.75rem] w-[15.625rem] rounded-2xl border-2 border-dashed bg-brand-50 px-4 py-3 shadow-card", selected ? "border-brand-500" : "border-brand-200")}>
      ...
      <div className="flex items-center gap-3 overflow-hidden"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-brand-100 text-brand-700">⑂</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-ink">{data.branchId}</p><p className="truncate text-3xs text-ink-mute">{data.componentCount} {t("workflow steps collapsed")}</p></div></div>
      <button type="button" className="mt-2 text-3xs font-semibold text-brand-700" onClick={(event) => { event.stopPropagation(); data.onExpand(data.branchId); }}>{t("Expand branch")}</button>
    </article>
  );
}
...
export function PipelineBuilder({ runId = null, sourceId = null, baseArtifactId = null, blueprint, layout, componentOutputs = [], onSaved, onChange, onExitAdvanced }: {
  ...
  const diagnosticIds = useDiagnosticIds(runId);
  ...
  return (
    <section className="relative flex h-full min-h-[34rem] flex-col overflow-hidden bg-surface-sunken">
      <header className="z-20 flex shrink-0 flex-wrap items-center gap-2 border-b border-line bg-surface px-4 py-2.5">
        ...
        <div className="mr-auto"><h2 className="text-sm font-semibold text-ink">{t("Workflow")}</h2><p className="text-3xs text-ink-mute">{t("The accepted graph defines the plan; layout is visual state only.")}</p></div>
      ...
      {selectedComponent && <Drawer side="right" title={t("Component inspector")} onClose={() => setSelected(null)}><ComponentInspector component={selectedComponent} blueprint={draft} engines={engines} outputs={componentOutputs.filter((output) => output.component_id === selectedComponent.id)} diagnosticIds={diagnosticIds} onRemove={() => removeComponent(selectedComponent.id)} onChange={(update) => updateComponent(selectedComponent.id, update)} /></Drawer>}
      {plannerOpen && <div className="absolute inset-y-0 right-0 z-40 flex w-[min(23.75rem,92vw)] pt-[3.3125rem] shadow-2xl"><PlannerPanel runId={runId} sourceId={sourceId} open onToggle={() => setPlannerOpen(false)} starterPrompts={[t("What columns are in this data?"), t("Rank the best target columns and ML problems."), t("Which relationships matter for prediction?")]} /></div>}
    </section>
  );
}

function Drawer({ side, title, onClose, children }: { side: "left" | "right"; title: string; onClose: () => void; children: React.ReactNode }) {
  return <aside className={`absolute inset-y-0 z-40 flex w-[min(23.75rem,92vw)] flex-col bg-surface pt-[3.3125rem] shadow-2xl ${side === "left" ? "left-0 border-r border-line" : "right-0 border-l border-line"}`}><header className="flex h-12 shrink-0 items-center border-b border-line px-4"><h3 className="flex-1 text-sm font-semibold text-ink">{title}</h3><button type="button" className="btn-ghost !px-2 !py-1" onClick={onClose}>×</button></header><div className="min-h-0 flex-1 overflow-y-auto p-4">{children}</div></aside>;
}
...
function ComponentLibrary({ catalog, onAdd }: { catalog: AutomationComponentDefinition[]; onAdd: (definition: AutomationComponentDefinition) => void }) {
  ...
  return <div><p className="text-xs leading-relaxed text-ink-mute">{t("Components expose registered contracts. Templates and visual groups are not runtime nodes.")}</p><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("Search components")} className="field mt-3 text-xs" /><div className="mt-5 space-y-5">{categories.map((category) => <section key={category}><p className="mb-2 text-3xs font-semibold uppercase tracking-wide text-ink-faint">{t(category)}</p><div className="space-y-2">{visible.filter((item) => item.category === category).map((item) => <button key={item.catalog_id} type="button" onClick={() => onAdd(item)} className="w-full rounded-xl border border-line px-3 py-2.5 text-left hover:border-brand-300 hover:bg-brand-50"><p className="text-xs font-semibold text-ink">{local(item.title)}</p><p className="mt-1 line-clamp-2 text-3xs leading-relaxed text-ink-mute">{local(item.description)}</p></button>)}</div></section>)}</div></div>;
}

/** #424: the Outputs list here enumerated every `artifact_ids` entry per port,
 *  so the advanced editor was one more surface that ignored the diagnostics
 *  preference entirely. It has a run id but no progress poller of its own --
 *  the blueprint is read rather than watched -- so it asks for the run's
 *  diagnostic ids once and filters against those. */
function ComponentInspector({ component, blueprint, engines, outputs, diagnosticIds, onRemove, onChange }: { component: PipelineComponent; blueprint: PipelineBlueprint; engines: DocumentEngine[]; outputs: PipelineOutputReference[]; diagnosticIds: ReadonlySet<string>; onRemove: () => void; onChange: (update: (component: PipelineComponent) => PipelineComponent) => void }) {
  const [preview, setPreview] = useState<ArtifactPreview | null>(null);
  const showDiagnostics = useShowDiagnostics();
  const visibleIds = (ids: string[]) => (showDiagnostics ? ids : ids.filter((id) => !diagnosticIds.has(id)));
