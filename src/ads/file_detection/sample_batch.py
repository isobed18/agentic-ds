"""Build the mixed-format regression corpus deterministically.

Binary fixtures such as ZIP, XLSX, PDF, and Parquet are generated at runtime so
the repository stays small while routing and escalation measurements remain
reproducible.
"""
...
def _pdf_bytes(satirlar: list[str]) -> bytes:
...
def write_sample_batch(kok: Path) -> Path:
    ...
    (kok / "rapor.pdf").write_bytes(_pdf_bytes([
    ...
    (kok / "arsiv.zip").write_bytes(_zip_bytes())
    (kok / "tablo.xlsx").write_bytes(_xlsx_bytes())
    (kok / "olcumler.parquet").write_bytes(_parquet_bytes())
