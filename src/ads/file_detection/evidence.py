"""Deterministic byte and text evidence.

CP1252 and CP1254 differ at six byte positions, while six other Turkish
letters occupy the same positions in both encodings. Shared Turkish bytes can
therefore provide evidence for how the differing bytes should be decoded.
This is byte evidence, not language-model inference.
"""
...
def detect_encoding(ham: bytes) -> EncodingResult:
    ...
    if ham.startswith(b"\xef\xbb\xbf"):
        return EncodingResult("utf-8-sig", "kesin", ["utf-8-sig"],
    ...
    if cp1252_ozel:
        ...
        return EncodingResult("cp1252", "yuksek", ["cp1252", "latin-1"], kanitlar)
    ...
    if yabanci_puan >= 2:
        ...
        return EncodingResult("cp1252", "yuksek",
...
def detect_number_format(degerler: list[str]) -> NumberFormatResult:
    ...
    if tr == 0 and en == 0:
        return NumberFormatResult(None, None, "yok",
                              belirsiz_adet=belirsiz)

    if tr and not en:
        return NumberFormatResult(",", ".", "yuksek", tr, en, belirsiz, tr_ornek)
    if en and not tr:
        return NumberFormatResult(".", ",", "yuksek", tr, en, belirsiz, en_ornek)
...
def detect_delimiter(ornek: str) -> DelimiterResult:
    ...
    if tutarli:
        ...
        return DelimiterResult(secilen, "yuksek", sayimlar)
